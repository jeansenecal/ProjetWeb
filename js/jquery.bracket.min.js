...
var __extends = this && this.__extends || function (a, b) {
  ...
  !function (a) {
	...
	function g(b, c, d) {
	  ...
	  g = e.winner(),
	  h = e.loser();
	  g && h && (g.name.isEmpty() || f(g.seed.get(), 'highlightWinner', b).highlight(), h.name.isEmpty() || f(h.seed.get(), 'highlightLoser', b).highlight()),
	...
	function r(c, d, e, f, g, h, i, j, k, l) {
	  ...
	  n = a('<div class="score" style="width: ' + i.scoreWidth + 'px;" ' + m + '/>'),
	  ...
	  }), r = a('<div class="team" style="width: ' + (i.teamWidth + i.scoreWidth) + 'px;"/>'), s = a('<div class="label" style="width: ' + i.teamWidth + 'px;"/>').appendTo(r); if (i.decorator.render(s, e.name.toNull(), p, q), e.seed.forEach(function (a) {
	...
	function s(b, c, d) {
	  var e = a('<div class="tools"/>').appendTo(b),
	  f = a('<span class="increment">+</span>').appendTo(e);
	  if (f.click(function () {
		for (var a = c.teams.length, b = 0; b < a; b += 1) c.teams.push([u.empty(),
		u.empty()]);
		return K(d)
	  }), c.teams.length > 1 && 1 === c.results.length || c.teams.length > 2 && 3 === c.results.length) {
		var g = a('<span class="decrement">-</span>').appendTo(e);
		g.click(function () {
		  if (c.teams.length > 1) return c.teams = c.teams.slice(0, c.teams.length / 2),
		  K(d)
		})
	  }
	  if (1 === c.results.length && c.teams.length > 1) {
		...
		h.click(function () {
		  if (c.teams.length > 1 && c.results.length < 3) return c.results.push([], [
		  ]),
		  K(d)
		})
		...
		h.click(function () {
		  if (3 === c.results.length) return c.results = c.results.slice(0, 1),
		  K(d)
		})
	  }
	}
	...
	z = function () {
	  ...
	  return a.teamsInResultOrder = function (a) {
		...
		c = a.b.name.isEmpty();
		if (c && !b) return a.b.emptyBranch() === t.BYE ? [
		  a.a,
		  a.b
		] : [
		];
		if (b && !c) return a.a.emptyBranch() === t.BYE ? [
		  a.b,
		  a.a
		] : [
		];
		if (!a.a.score.isEmpty() && !a.b.score.isEmpty()) {
		  if (a.a.score.get() > a.b.score.get()) return [a.a,
		  a.b];
		  if (a.a.score.get() < a.b.score.get()) return [a.b,
		  a.a]
		}
		return []
	  },
	...
	E = function () {
	  ...
	  b.prototype.render = function () {
		this.roundCon.empty(),
		(this.doRenderCb.isEmpty() || this.doRenderCb.get() ()) && (this.roundCon.appendTo(this.bracket.el), this.matches.forEach(function (a) {
		  return a.render()
		}))
	  },
	...
	H = function () {
	  function c(c, d, e, f, g, h, i, j, k, l) {
		if (this.round = c, this.match = d, this.seed = e, this.renderCb = g, this.isFirstBracket = h, this.opts = i, this.resultId = j, this.topCon = k, this.renderAll = l, this.connectorCb = u.empty(), this.matchCon = a('<div class="match"/>'), this.teamCon = a('<div class="teamContainer"/>'), this.alignCb = null, this.matchUserData = f.isEmpty() ? void 0 : f.get().userData, !i.save) {
	...
	K = function (b) {
	  ...
	  function d(a) {
		...
		a && (j.results[0] = e.results(), f && (j.results[1] = f.results()), h && !b.skipGrandFinalComeback && (j.results[2] = h.results()), c(), b.save && b.save(q(j), b.userData))
	  }
	  var e,
	  f,
	  h,
	  ...
	  r = a('<div class="jQBracket ' + b.dir + '"/>').appendTo(b.el.empty());
	  ...
	  v,
	  w;
	  k ? v = a('<div class="bracket"/>').appendTo(r)  : (b.skipGrandFinalComeback || (t = a('<div class="finals"/>').appendTo(r)), v = a('<div class="bracket"/>').appendTo(r), w = a('<div class="loserBracket"/>').appendTo(r)),
	  v.css('height', o),
	  ...
	  k || (f = new F(w, u.of(j.results[1] || null), x, !1, b), b.skipGrandFinalComeback || (h = new F(t, u.of(j.results[2] || null), x, !1, b))),
	  l(e, j.teams, k, b, b.skipGrandFinalComeback && !k),
	  k || (m(e, f, j.teams.length, b.skipGrandFinalComeback, b.centerConnectors), b.skipGrandFinalComeback || n(h, e, f, b, r, c)),
	  d(!1),
	  {
		data: function () {
		  return q(b.init)
	...
	P = {
	  init: function (b) {
		...
		c.dir = c.dir || 'lr',
		c.init.teams = c.init.teams && 0 !== c.init.teams.length ? c.init.teams : [
		  [null,
		  null]
		],
